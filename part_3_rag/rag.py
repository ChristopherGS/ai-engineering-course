...
from llama_index import Response, set_global_tokenizer, ServiceContext, VectorStoreIndex, SimpleDirectoryReader, \
...
def save_or_load_index(
    ...
    transcript_files = [_file for _file in transcript_files if "test_data" not in Path(_file).parts]
...
def main():
    ...
    rag_result = ChatResponse = run_inference(
        llm=llm, embedding_model=embedding_model, use_rag=True, messages=[system_prompt, user_prompt]
    )
